...
const Flt COMPLETE_ANGLE = 360.0;
const int HALF_ANGLE = 90.0;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+90.0) / COMPLETE_ANGLE) * PI * 2.0;
...
void physics()
{
	...
	if (gl.keys[XK_Up]) {
		//apply thrust
		//convert ship angle to radians
		Flt rad = ((g.ship.angle+HALF_ANGLE) / COMPLETE_ANGLE) * PI * 2.0;
		//convert angle to a vector
		Flt xdir = cos(rad);
		Flt ydir = sin(rad);
...
void render()
{
	...
	ggprint8b(&r, 16, 0x00ffff00, "n asteroids destroyed: %i", g.nasteroids_del);
	//
	//-------------
	//Draw the ship
